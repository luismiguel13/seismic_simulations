...
def fastMarching_approach(data_path, data_format='numpy', exp_number=1, H=None, alpha=None):
    """
    Parameters
    ----------
    ...
    rem_shots = rem_shots[pattern_rand == 0]
    print(rem_shots)
    for i in range(len(rem_shots)):
        psnr_list.append(PSNR(x[..., rem_shots[i]], output[..., rem_shots[i]]))
        ssim_list.append(ssim(x[..., rem_shots[i]], output[..., rem_shots[i]]))
